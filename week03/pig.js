...
var getRandomNumber = function(max) {
	...
	if (!isNaN(max)) {
		...
		random = Math.floor(random * max);	//integer between 0 and max -1
		random = random + 1;				//integer between 1 and max
	}
	return random;
};
var changePlayer = function() {
	...
	} else { $("current").firstChild.nodeValue = $("player1").value; }
	$("die").value = "0";
	$("total").value = "0";
	$("roll").focus();
};
...
var holdTurn = function() {
	...
	if (score.value >= 100) {
		alert($("current").firstChild.firstChild.nodeValue + " WINS!");
		newGame();
	} else { changePlayer(); }
};
